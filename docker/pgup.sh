...
#########################################################################
PGDARADIR=${PGDARADIR:-"/var/lib/postgresql/data"}
...
if [ ! -e "${PGDARADIR}/postgresql.conf" ]; then
    ...
    su - postgres -c "initdb --username=postgres --encoding=UTF8 --data-checksums ${PGDARADIR}"
    su - postgres -c "pg_ctl start -D ${PGDARADIR}"
    su - postgres -c "psql -c \"CREATE USER ${POSTGRES_USER} WITH PASSWORD '${POSTGRES_PASSWORD}';\""
    su - postgres -c "psql -c \"CREATE DATABASE ${POSTGRES_DB} OWNER ${POSTGRES_USER}\";"
    su - postgres -c "psql -c \"GRANT ALL PRIVILEGES ON DATABASE ${POSTGRES_DB} TO ${POSTGRES_USER};\""
    su - postgres -c "pg_ctl stop -D ${PGDARADIR}" 
    sed -i \$a'host all  all    0.0.0.0/0   md5' "${PGDARADIR}/pg_hba.conf"
    sed -i \$a"listen_addresses = '*'" "${PGDARADIR}/postgresql.conf"
fi

su - postgres -c "postgres -D ${PGDARADIR}" 
