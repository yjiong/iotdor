...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withAdmins; query != nil {
		if err := gq.loadAdmins(ctx, query, nodes,
			func(n *Group) { n.Edges.Admins = []*User{} },
			func(n *Group, e *User) { n.Edges.Admins = append(n.Edges.Admins, e) }); err != nil {
			return nil, err
		}
	}
	if query := gq.withGateways; query != nil {
		if err := gq.loadGateways(ctx, query, nodes,
			func(n *Group) { n.Edges.Gateways = []*Gateway{} },
			func(n *Group, e *Gateway) { n.Edges.Gateways = append(n.Edges.Gateways, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadAdmins(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Group)
	nids := make(map[int]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(group.AdminsPrimaryKey[1]))
...
func (gq *GroupQuery) loadGateways(ctx context.Context, query *GatewayQuery, nodes []*Group, init func(*Group), assign func(*Group, *Gateway)) error {
	...
	for _, n := range neighbors {
		fk := n.group_gateways
		if fk == nil {
			return fmt.Errorf(`foreign-key "group_gateways" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_gateways" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
