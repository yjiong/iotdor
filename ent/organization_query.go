...
func (oq *OrganizationQuery) QueryDevices() *DeviceQuery {
	query := &DeviceQuery{config: oq.config}
...
func (oq *OrganizationQuery) WithDevices(opts ...func(*DeviceQuery)) *OrganizationQuery {
	query := &DeviceQuery{config: oq.config}
...
func (oq *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := oq.withDevices; query != nil {
...
func (oq *OrganizationQuery) loadDevices(ctx context.Context, query *DeviceQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Device)) error {
	...
	query.Where(predicate.Device(func(s *sql.Selector) {
		s.Where(sql.InValues(organization.DevicesColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.organization_devices
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_devices" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
