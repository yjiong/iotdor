...
func (dq *DeviceQuery) QueryGateway() *GatewayQuery {
	query := &GatewayQuery{config: dq.config}
...
// the "gateway" edge. The optional arguments are used to configure the query builder of the edge.
func (dq *DeviceQuery) WithGateway(opts ...func(*GatewayQuery)) *DeviceQuery {
	query := &GatewayQuery{config: dq.config}
	for _, opt := range opts {
		opt(query)
	}
	dq.withGateway = query
	return dq
}
...
func (dq *DeviceQuery) sqlAll(ctx context.Context) ([]*Device, error) {
	...
	if query := dq.withGateway; query != nil {
		...
		for i := range nodes {
			if nodes[i].gateway_devices == nil {
				continue
			}
			fk := *nodes[i].gateway_devices
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(gateway.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "gateway_devices" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Gateway = n
			}
		}
	}
